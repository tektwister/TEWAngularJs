...
export class ParticipantsComponent implements OnInit {
  ...
  onSubmit(values: any) {
    ...
    if (this.participantsForm.valid) {
      ...
      this.partService.createParticipant(email_id, userProfileName,"Participant").subscribe((response: any) => {
        if (response.success) {
  ...
  getParticipants() {
    this.partService.getAllParticipants("Participant").subscribe((response:any) => {
